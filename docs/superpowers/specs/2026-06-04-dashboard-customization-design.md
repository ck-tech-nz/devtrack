...
嵌入地址(含 key)是**单一完整值** —— 即 iframe `src` 整条(`http://host:9300/embed?key=...`),URL 与 key 不分开配置。

- `frontend/nuxt.config.ts`:`runtimeConfig.public` 增加 `serverMonitorUrl: ''`(空串 → 运行时由 `NUXT_PUBLIC_SERVER_MONITOR_URL` 覆盖)。
- **本地开发**:在 `frontend/.env` 追加一行(改后重启 `npm run dev`,env 在启动时读取):
  ```bash
  NUXT_PUBLIC_SERVER_MONITOR_URL=http://localhost:9300/embed?key=smk_-jNiwEc4r9Im9IfsA9uEaxp0GqxqUxYW
  ```
- **部署(test/prod,Docker)**:`docker-compose.yml` 的 `frontend` 服务目前**没有** `environment:` 段,需新增一条把变量作为**运行时 env**(不是 `build.args`)传入容器:
  ```yaml
  frontend:
    build:
      context: ./frontend
    ...
    ports:
      - "3000:3000"
  ```
  再在各服务器 compose 读取的 `.env` 中给出该环境对应地址(test/prod 各自不同)。因前端是 Nitro node server、运行时才读该变量,**同一镜像无需重新构建**即可在不同环境显示不同地址。
- **安全提示**:`NUXT_PUBLIC_*` 会注入到浏览器页面,该地址连同 key 对任何登录用户可见(查看源码即可见)。鉴于「所有人可见」决策且这是只读监控嵌入 key,可接受;若该 key 实际具备修改服务器配置等写权限,则需改为后端代理方案(本期不做)。
...
- `frontend/.env`(本地)追加 `NUXT_PUBLIC_SERVER_MONITOR_URL`
- `docker-compose.yml` — `frontend` 服务新增 `environment:` 段传入 `NUXT_PUBLIC_SERVER_MONITOR_URL`(部署环境的 `.env` 提供实际值)
