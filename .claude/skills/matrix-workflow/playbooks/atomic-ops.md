# atomic-ops — shared subroutines for matrix-workflow playbooks

Other playbooks reference these subroutines by name. They are not standalone commands.

## identify-repo

This is a single-repo project, rooted at `./`. No monorepo sub-repos.

1. Ensure the current working directory is inside this repo (`git rev-parse --show-toplevel`). If not, `cd` to the repo root before continuing.
2. No repo-choice prompt is needed.
