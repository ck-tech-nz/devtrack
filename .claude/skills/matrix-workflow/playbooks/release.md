...
There is **no build step** — deployment is simply a push to `env/test` or `env/prod`. The downstream CI/CD picks it up.

## Tag format

| Environment | Tag format |
|---|---|
| `env/prod` | `v{date}[-{suffix}]` (e.g., `v2026-04-21`, `v2026-04-21-2` if today's tag exists) |
| `env/test` | no tag |

Release-note files live at `docs/releases/` and are organized **one file per calendar month** — `{YYYY-MM}.md` — with newest releases at the top.

## Flow

1. **Parse target.** Extract environment (`test` or `prod`). Ask if unclear.
2. **identify-repo** (see `atomic-ops.md`) — ensures we're at the repo root.
...
7. **Aggregate release notes into the month file.** Skip this step when environment is `test` OR when source ≠ main (there are no PRs to aggregate against main).
   ...
   3. Locate the month file `docs/releases/{YYYY-MM}.md` where `{YYYY-MM}` is derived from the new tag's date.
...
9. Report:
   ...
   ```
   Released: env/<environment>
   Source: <source-branch> (pre-merge test push)         # only when source ≠ main
   Commit: <hash> <message>
   Tag: <tag>                                            # omit for env/test
   Release notes: docs/releases/{YYYY-MM}.md             # omit for env/test or pre-merge pushes
   ```
