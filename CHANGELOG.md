...
## 2026-06-12 — 问题列表标题列支持拖拽调宽

问题列表「标题」列支持拖拽调整列宽,宽度按浏览器记忆、刷新后保留。

### 新增

- **标题列宽可调**:问题列表「标题」表头右侧新增拖拽抓手(双竖条握柄,常驻可见、hover 点亮主题紫并浮出柔光底),可左右拖拽调整标题列宽,原因分析/解决方案两列自动让位。宽度按浏览器存入 `localStorage`(键 `issues:title-col-width`,夹在 120–700px),刷新后保留;双击抓手复原默认列宽。新增 `useColumnWidth` 组合式与 `columnWidth` 纯函数(含单测),列宽改由 `--title-col-w` CSS 变量驱动,不再写死。

## 2026-06-12 — 走马灯公告纳入 Django admin + 页脚构建时间 + 问题列表精简

走马灯公告补齐后台管理入口;页脚版本号补充构建时间;问题列表移除「备注」列。

### 新增

- **走马灯公告 Django admin**:`Bulletin` 模型注册进后台（按分类/启用筛选,列表内直接切换启用与排序,创建人自动记录当前管理员）;UNFOLD 侧边栏「通知」分组新增「走马灯公告」入口。此前该模型只有前端「走马灯管理」页与 API,后台无管理入口。

### 变更

- 页脚版本号由 `env/<env> <sha>` 扩展为 `env/<env> <构建时间> <sha>`（`gitHash`/`buildDate` 缺失时回退原值）。
- 问题列表表格移除「备注」列(含行内编辑单元格与列宽规则),腾出空间给标题/原因/方案。
